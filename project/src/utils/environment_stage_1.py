...
class Env():
    def __init__(self, action_dim=2, train=True):
        self.goal_x = 0
        self.goal_y = 0
        self.heading = 0
        self.initGoal = True
    ...
    def getState(self, scan, past_action):
        ...
        if self.train: # if training
            if current_distance < 0.30: #0.15:
    ...
    def setReward(self, state, done):
        ...
        if done:
            ...
            if world:
                ...
                self.goal_x, self.goal_y = self.respawn_goal.getPosition(True, delete=True, train=self.train) # if validation -> Modify "train = False"
                #self.respawn_goal.reset_path()
        ...
        return reward, done, reached_goal_flag
    ...
    def step(self, action, past_action):
        ...
        reward, done, reached_goal_flag = self.setReward(state, done)

        return np.asarray(state), reward, done, reached_goal_flag
