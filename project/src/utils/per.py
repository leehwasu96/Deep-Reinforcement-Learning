...
class ReplayBuffer(object):
    ...
    def _encode_sample(self, idxes):
        ...
        for exp in exps:
            ...
            s_.append(torch.from_numpy(np.array(s)).clone())
            a_.append(torch.from_numpy(np.array(a)).clone())
            r_.append(torch.tensor(r, dtype=torch.float32))
            s2_.append(torch.from_numpy(np.array(s2)).clone())
            gamma_.append(torch.tensor(gamma, dtype=torch.float32))
            flag_.append(flag)

        return torch.stack(s_), torch.stack(a_), torch.stack(r_), \
               torch.stack(s2_), torch.stack(gamma_), torch.tensor(flag_, dtype=torch.float32)
...
class PrioritizedReplayBuffer(ReplayBuffer):
    ...
    def add(self, experience):
        idx = self._next_idx
        super(PrioritizedReplayBuffer, self).add(experience)
