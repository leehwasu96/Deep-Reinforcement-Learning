...
from per2 import PrioritizedReplayBuffer
import per_utils as utils
#from respawnPath import RespawnPath
...
class Trainer:
    ...
    def optimizer(self):
        s_sample, a_sample, r_sample, new_s_sample, done_sample, weight_sample, idx_sample = ram.sample(BATCH_SIZE, beta_value)

        weight_sample = np.ones_like(r_sample)
        weight_sample = np.sqrt(weight_sample)
        
        s_sample = torch.from_numpy(s_sample).float()
        a_sample = torch.from_numpy(a_sample).float()
        r_sample = torch.from_numpy(r_sample).float()
        new_s_sample = torch.from_numpy(new_s_sample).float()
        done_sample = torch.from_numpy(done_sample).float()
        weight_sample = torch.from_numpy(weight_sample).float()
        ...
        weighted_td_errors = torch.mul(td_errors, weight_sample)
        ...
        loss_critic = F.mse_loss(weighted_td_errors,zero_tensor)
        ...
        ram.update_priorities(idx_sample, new_priorities)


    def save_models(self, episode):

        torch.save(self.target_actor.state_dict(), dirPath + '/Models/ddpg_per/hwasu_stage_3/new_result/' + str(episode) + '_actor.pt')

        torch.save(self.target_critic.state_dict(), dirPath + '/Models/ddpg_per/hwasu_stage_3/new_result/' + str(episode) + '_critic.pt')
    ...
    def load_models(self, episode):
        ...
        self.actor.load_state_dict(torch.load(dirPath + '/Models/ddpg_per/hwasu_stage_3/new_result/' + str(episode) + '_actor.pt'))

        self.critic.load_state_dict(torch.load(dirPath + '/Models/ddpg_per/hwasu_stage_3/new_result/' + str(episode)+ '_critic.pt'))
...
reward_path = dirPath + '/Models/ddpg_per/hwasu_stage_3/new_result/train_stage_3_reward_result.txt'
time_path = dirPath + '/Models/ddpg_per/hwasu_stage_3/new_result/train_stage_3_time_result.txt'
per_step_path = dirPath + '/Models/ddpg_per/hwasu_stage_3/new_result/per_step.txt'
...
#PER parameters
prioritized = True
beta_value = 0 # init beta value
prioritized_replay_alpha=0.6
prioritized_replay_beta0=0.4
prioritized_replay_beta_iters=None
prioritized_replay_eps=1e-6 # epsilon
ram = PrioritizedReplayBuffer(MAX_BUFFER, prioritized_replay_alpha)
if prioritized_replay_beta_iters is None:
    prioritized_replay_beta_iters = MAX_EPISODES * MAX_STEPS #train_steps
...
load_model_episode = 1680 # 2000
trainer.load_models(episode=load_model_episode)

if __name__ == '__main__':
    rospy.init_node('ddpg_per_stage_3')
    ...
    for ep in range(load_model_episode+1, MAX_EPISODES+1):
        ...
        for step in range(1, MAX_STEPS+1):
            ...
            if (done) or (step == MAX_STEPS) or (reached_goal_flag_cnt == 10):
                ...
                beta_value = 0
                beta_value = beta_schedule.value(per_step)
                ...
                save_result([per_step], per_step_path)
