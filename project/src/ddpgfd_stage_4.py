...
class Trainer:
    ...
    def pretrain(self):
        ...
        update_step = 60000      #2000
        pretrain_step = 300000   #10000
        cnt = 0
        for step in np.arange(update_step, pretrain_step + 1, update_step): # total 5 step
            ...
            if pre_ram.len > 0:
                for _ in range(update_step): # total 2000 step
                    ...
                    #soft_update(self.target_critic, self.critic, TAU)
                    """
        self.save_models(0) # 300,000 step pretrain

    def save_models(self, episode):

        torch.save(self.target_actor.state_dict(), dirPath + '/Models/ddpgfd/hwasu_stage_4/new_result3/' + str(episode) + '_actor.pt')

        torch.save(self.target_critic.state_dict(), dirPath + '/Models/ddpgfd/hwasu_stage_4/new_result3/' + str(episode) + '_critic.pt')
            
        print('***** stage 4 -> Episode: '+str(episode)+' Model save... *****')
        
        #print('****Models saved***')
        
    def load_models(self, episode):

        self.actor.load_state_dict(torch.load(dirPath + '/Models/ddpgfd/hwasu_stage_4/new_result3/' + str(episode) + '_actor.pt'))

        self.critic.load_state_dict(torch.load(dirPath + '/Models/ddpgfd/hwasu_stage_4/new_result3/' + str(episode)+ '_critic.pt'))

        print('***** stage 4 -> Episode: '+str(episode)+' Model load... *****')
...
reward_path = dirPath + '/Models/ddpgfd/hwasu_stage_4/new_result3/train_stage_4_reward_result.txt'
time_path = dirPath + '/Models/ddpgfd/hwasu_stage_4/new_result3/train_stage_4_time_result.txt'
...
ram = PrioritizedReplayBuffer(MAX_BUFFER, alpha=0.3, beta_init=0.5, beta_inc_n=500, seed=1000) 
# alpha = 0.3 / beta_init = 1.0 / beta_inc_n = 100

# load demo data to PER #
demonstrations = load_demonstrations('/home/unicon1/catkin_ws/src/project/src/demonstrations_data_stage_4_10000goals.pkl')
...
#print("Pre Buffer size: "+str(pre_ram.len)) # 439,053
#print("Buffer size: "+str(len(ram)))        # 877,479
...
trainer.load_models(episode=load_model_episode)  # 0 is pretrain model load

if __name__ == '__main__':
    rospy.init_node('ddpgfd_stage_4')
